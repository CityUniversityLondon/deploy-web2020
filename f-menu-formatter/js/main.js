...
function prepareNavigation(navigation, rootClass) {
  ...
  Array.from(navigation.querySelectorAll('li')).forEach(navigationItem => {
    const link = navigationItem.querySelector('a'),
          ...
          linkText = 'Visit ' + navigationItem.dataset.title; //textWrapper = document.createElement('span'),
    ...
    link.title = linkText; //navigationItem.replaceChild(controlsWrapper, link);
...
function launchArrow(slider) {
  ...
  controlsWrapper.appendChild(nextButton);
  controlsWrapper.className = className + '__controls';
  controlsWrapper.setAttribute(_aria_attributes__WEBPACK_IMPORTED_MODULE_5__["default"].label, 'Slider navigation'); // Places controls directly after 'ul' containing the slides
