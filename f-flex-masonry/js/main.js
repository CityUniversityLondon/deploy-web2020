...
/* harmony import */ var _patterns_image_column_layout_image_column_layout__WEBPACK_IMPORTED_MODULE_29__ = __webpack_require__(/*! ./patterns/image-column-layout/image-column-layout */ "./src/patterns/image-column-layout/image-column-layout.js");
...
var className = 'imageColLayout'; // Check if images loaded and resolve it in a Promise

var checkImg = path => new Promise(resolve => {
  var img = new Image();
...
function init(elem) {
  var columns = 4,
      images = [].slice.call(elem.querySelectorAll('img')).map(i => i.src); // Promise object
  ...
  var loadImg = function loadImg() {
    ...
    return Promise.all(paths.map(checkImg));
  }; // When resolve calculate elements height


  loadImg(images).then(() => {
    var items = [].slice.call(elem.querySelectorAll('.imageColLayout--card'));
    var total = items.map(el => {
      return el.offsetHeight;
    });
    var add = total.reduce((a, c) => a + c, 0);
    var rows = (items.length + 1) / columns;
    var a = add / (items.length + 1);
    elem.style.setProperty('--h', a * Math.ceil(rows) + 'px');
  });
}
