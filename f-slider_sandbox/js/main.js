...
/* harmony import */ var _patterns_slider_slider__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! ./patterns/slider/slider */ "./src/patterns/slider/slider.js");
...
function configureInitialProgressIndicator(sliderElement) {
  let sliderChildren, sliderCollectionLength;
  const slideIndex = 1;
  sliderChildren = getActiveSliderChildren(sliderElement);
  sliderCollectionLength = getSliderCollectionLength(sliderChildren);
...
function configureActiveSlide(sliderTarget, slidesCollection, slideIndex) {
  if (sliderTarget !== '.slider--image-carousel') {
    let currentSlide = slidesCollection[slideIndex];
    currentSlide.classList.add('slider__slide--active');
  } else {
    for (let i = 0; i < slidesCollection.length; i++) {
      let dataIndex = parseInt(slidesCollection[i].getAttribute('data-index'));

      if (dataIndex == 0) {
        slidesCollection[i].setAttribute('data-index', '2');
      } else {
        dataIndex--;
        slidesCollection[i].setAttribute('data-index', dataIndex);
      }
    }
  }
}
...
function setProgressIndicator(sliderElement, slideIndexDisplay, sliderCollectionLength) {
  let progressIndicatorElement = sliderElement.querySelector('.slider__controls__progress-indicator__progress');
  let percentageProgress = getPercentageProgress(slideIndexDisplay, sliderCollectionLength);
  progressIndicatorElement.setAttribute('style', 'width: ' + percentageProgress + '%');
}

function getPercentageProgress(slideIndexDisplay, sliderCollectionLength) {
