...
/* harmony import */ var _logic_params_match__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../../logic/params-match */ "./src/patterns/finder/logic/params-match.js");
...
function Finder__Results(props) {
  ...
  if (props.response) {
    ...
    const featureCards = Object(_logic_params_match__WEBPACK_IMPORTED_MODULE_8__["queryIsEqualToConfigFeatureCard"])(props.query, props.config.featuredCard) && props.response.extraSearches.feature ? props.response.extraSearches.feature.response.resultPacket.results.map(card => /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_6___default.a.createElement(_cards_finder_results_card__WEBPACK_IMPORTED_MODULE_1__["default"], {
      bestBet: false,
      details: card,
      key: card.docNum,
      type: props.type,
    ...
    if (props.response.results && !promoCardConditionMet) {
      results = props.response.bestBets.length > 0 || props.response.results.length > 0 ? /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_6___default.a.createElement("ol", {
        start: props.response.summary.currStart,
        className: resultsClass
      }, featureCards, props.response.bestBets.map(card => /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_6___default.a.createElement(_cards_finder_results_bestbet__WEBPACK_IMPORTED_MODULE_0__["default"], {
        details: card,
        key: card.docNum
      })), props.response.results.map(card => /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_6___default.a.createElement(_cards_finder_results_card__WEBPACK_IMPORTED_MODULE_1__["default"], {
        bestBet: false,
...
Finder__Results.propTypes = {
  ...
  promo: prop_types__WEBPACK_IMPORTED_MODULE_5___default.a.object
};
...
/***/ "./src/patterns/finder/logic/params-match.js":
/*!***************************************************!*\
  !*** ./src/patterns/finder/logic/params-match.js ***!
  \***************************************************/
/*! exports provided: queryIsEqualToConfigFeatureCard */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "queryIsEqualToConfigFeatureCard", function() { return queryIsEqualToConfigFeatureCard; });


/**
 * Checks if the given query object are equal to features cards configure
 *
 * @param {object} query Object of current query
 * @param {object} config Object of current feature cards configuration
 * @return {boolean} - Return true if the query matches the features cards
 */
function queryIsEqualToConfigFeatureCard(query, config) {
  const facetsMap = Object.keys(query.facets),
        facetValuesMap = Object.values(query.facets),
        curQuery = query.query,
        configFacets = config.facets,
        filterCofigFacets = configFacets.filter(facet => facetsMap.filter(f => facet[f]).length > 0),
        parametersExist = filterCofigFacets.filter(facet => facetValuesMap.filter(f => f === Object.values(facet)[0]).length > 0),
        anyParam = filterCofigFacets.filter(facet => {
    const value = Object.values(facet)[0];
    return value === 'any';
  });

  if (curQuery === config.query) {
    return true;
  }

  if (parametersExist.length) {
    return true;
  }
