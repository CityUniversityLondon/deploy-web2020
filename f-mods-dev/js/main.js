...
const className = 'modal',
      bodyClassName = className + '__body',
      veilClassName = className + '__veil'; // let trap;
...
function createDialog(parent, position, dialogArray) {
  ...
  document.querySelector('body').appendChild(dialog); // const modalBackground = document.createElement('div');
  ...
  const trap = focus_trap__WEBPACK_IMPORTED_MODULE_1___default()(dialog, {
    clickOutsideDeactivates: true
  });
  trap.activate(); // document.body.classList.add('modal--in', 'no-scroll');
...
function closeDialog(dialog, trap) {
  dialog.parentNode.removeChild(dialog); // background.parentNode.removeChild(background);
