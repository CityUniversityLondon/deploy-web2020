...
function launchAccordion(accordion) {
  const locationHash = window.location.hash.substr(1),
        toggleOpen = Object(_util__WEBPACK_IMPORTED_MODULE_2__["toBool"])(accordion.dataset.toggleopen),
...
const className = 'pagination',
      ...
      buttonsAlwaysDisplayed = 1,
      buttonsDisplayedWithin = 2,
      scrollDuration = Object(_util__WEBPACK_IMPORTED_MODULE_3__["reduceMotion"])() ? 0 : 999;
/**
...
function createToggleNextPrev(next, prev, pageCount) {
  /***
   * Disables the next/previous page buttons if there is no next/previous page relative to this page.
   *
...
/**
 * We don't want overwhelmingly long lists of page buttons, so hide all but a
 * few to either side of the current page.
 *
 * @param {number} pageCount - The total number of pages in this paginated element.
...
function hideTooManyButtons(pageCount, controls, pageNumber) {
  if (pageCount > maximumButtonsToDisplay) {
    controls.forEach(function (button) {
      if (!button.className.match(nextPrevRegEx)) {
        ...
        /**
         * We always display the next/previous, first and last buttons,
         * and up to two additional page buttons to either side of the
         * current page.
         */

        if (buttonNumber <= buttonsAlwaysDisplayed || buttonNumber > pageCount - buttonsAlwaysDisplayed || Math.abs(pageNumber - buttonNumber) <= buttonsDisplayedWithin) {
          button.toggleAttribute(_aria_attributes__WEBPACK_IMPORTED_MODULE_4__["default"].hidden, false);
          button.toggleAttribute('disabled', false);
        } else {
          button.toggleAttribute(_aria_attributes__WEBPACK_IMPORTED_MODULE_4__["default"].hidden, true);
...
function createOpenPage(pages, controls, toggleNextPrev) {
   ...
   */
  const openPage = (pageNumber, scrollTo) => {
    ...
    hideTooManyButtons(pagesArray.length, controlsArray, pageNumber);
...
function createNextPrevOpenPage(controls, newPageFn, openPage) {
  ...
  const nextPrevOpenPage = () => {
    ...
    openPage(newPageFn(currentPage), true);
  };
...
function createPageButton(pageNumber, totalPages) {
  ...
  if (totalPages - pageNumber === 1) {
    button.className = buttonClassName + '--penultimate';
  }
...
function addPagination(elem, itemCount) {
  ...
  if (!pages.length > 1) {
    /**
     * too few poges, doesn’t need pagination.
     * remove the classname to avoid conflicting
     * styles then bail
     */
  ...
  for (let pageNumber = 1; pageNumber <= pages.length; pageNumber++) {
    ...
    pageButton.addEventListener('click', () => openPage(pageNumber, true));
...
function toggleLink(link, selected) {
  link.setAttribute(_aria_attributes__WEBPACK_IMPORTED_MODULE_3__["default"].selected, selected);
  selected ? link.removeAttribute('tabindex') : link.setAttribute('tabindex', -1);
}
/**
...
function selectTab(e, newTab) {
  ...
  /**
   * Move focus to the section and scroll it into view.
   */

  newTab.focus();
  zenscroll__WEBPACK_IMPORTED_MODULE_1___default.a.to(tabs, scrollDuration);
}
/**
...
function keyEvents(e, tabs) {
  const currentTab = tabs.querySelector(`[${_aria_attributes__WEBPACK_IMPORTED_MODULE_3__["default"].selected}="true"]`),
...
function prepareLinks(linkItems) {
  ...
  linkItems.forEach(linkItem => {
    ...
    if (link.hash === window.location.hash) {
      idLinked = window.location.hash;
      toggleLink(link, true);
    } else {
      toggleLink(link, false);
    }

    link.addEventListener('click', e => selectTab(e, link), true);
  });
  return idLinked;
}
/**
...
function launchTabs(tabs) {
  ...
  if (!idLinked) {
    ...
    tabs.querySelector(idLinked).toggleAttribute('hidden', false);
  }
  /**
