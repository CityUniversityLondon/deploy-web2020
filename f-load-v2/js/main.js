...
const launchLoadMore = e => {
  const increment = parseInt(e.dataset.inc),
        wrapperId = e.id,
        items = Array.from(e.querySelectorAll('.load-more__item'));
...
const updateVisible = (wrapper, increment) => {
  ...
  if (increment < hiddenElements.length) {
    itemsToShow = increment;
  } else {
    itemsToShow = hiddenElements.length;
    let button = wrapper.querySelectorAll('.load-more__button');
    wrapper.removeChild(button[0]);
  }
  ...
  updateHashURL(wrapperId, visibleItems.length);
};
